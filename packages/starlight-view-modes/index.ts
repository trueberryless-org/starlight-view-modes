      ...
      setup({ addIntegration, config, logger, updateConfig }) {
        ...
        if (config.components?.PageSidebar) {
          ...
          updatedConfig.components.PageSidebar =
            './src/plugins/view-modes/overrides/PageSidebar.astro';
        }
        if (config.components?.Sidebar) {
          ...
          logger.warn(
            'To render `@astrojs/starlight-view-modes`, remove the override for the `Sidebar` component.\n'
          );
        } else {
          // Otherwise, add the Sidebar component override to the user's configuration.
          updatedConfig.components.Sidebar =
            './src/plugins/view-modes/overrides/Sidebar.astro';
        }
...
export type StarlightViewModesUserConfig = z.input<
  typeof starlightViewModesConfigSchema
>;
export type StarlightViewModesConfig = z.output<
  typeof starlightViewModesConfigSchema
>;
