...
export const AvailableModes: AvailableMode[] = [
  {
    ...
    switchToText: "starlightViewModes.switchToDefaultMode",
  },
  {
    name: ZenMode,
    title: "starlightViewModes.zenMode.title",
    switchToText: "starlightViewModes.switchToZenMode",
    enabled: config.zenModeSettings.enabled,
    exclude: getLocalizedExclude(config.zenModeSettings.exclude),
    enableIcon: zenModeEnableIcon,
    disableIcon: zenModeDisableIcon,
...
export function getAdditionalMode(name: string): AdditionalMode | undefined {
  return AdditionalModes.find((mode) => mode.name === name);
}

export function getModeShortcuts(
  shortcuts: string[],
  mode: string
): Shortcut[] {
  return shortcuts.map((shortcut) => parseShortcut(shortcut, mode));
}
