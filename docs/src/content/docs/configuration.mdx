...
:::note
We don't recommend using the shortcut `Ctrl+K` because it is already used for opening the search modal.
:::
...
Set this option to `2` to only start a new slide for `h2` headings and keep `h3` headings on the slide of their parent section.
